...
namespace RT_Rimtroid
{
    ...
    public static class RT_Utils
    {
        ...
        public static void ApplySettings()
        {
            ...
            {
                SetStat(RT_RimtroidDefOf.RT_AlphaMetroid, StatDefOf.Wildness, 0.99f);
                SetStat(RT_RimtroidDefOf.RT_GammaMetroid, StatDefOf.Wildness, 0.99f);
                SetStat(RT_RimtroidDefOf.RT_ZetaMetroid, StatDefOf.Wildness, 0.99f);
                SetStat(RT_RimtroidDefOf.RT_OmegaMetroid, StatDefOf.Wildness, 0.99f);
            }
        }
        private static void SetStat(ThingDef def, StatDef stat, float value)
        {
            ...
            var existing = def.statBases.FirstOrDefault(s => s.stat == stat);
            if (existing != null)
            {
