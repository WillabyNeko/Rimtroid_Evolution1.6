...
namespace RT_Rimtroid
{
    public class IncidentWorker_OmegaBecomesQueen : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            ...
            Log.Message("PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction.Any(x => x.IsOmegaMetroid()): " + PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive_OfPlayerFaction.Any(x => x.IsOmegaMetroid()));
            ...
            return (metroidFaction.leader is null || metroidFaction.leader.Dead) && (comp.currentQueen is null || comp.currentQueen.Dead)
                && PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive_OfPlayerFaction.Any(x => x.IsOmegaMetroid()) 
                && comp.lastQueenDeathTick != 0 && Find.TickManager.TicksGame > comp.lastQueenDeathTick + (GenDate.TicksPerDay * 15);
        }
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            var omegaToQueen = PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive_OfPlayerFaction.Where(x => x.IsOmegaMetroid()).RandomElement();
